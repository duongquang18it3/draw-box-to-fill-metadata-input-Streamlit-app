...
def save_to_json(file_base64, file_name, doc_type_id, metadata_values):
    ...
    data = {
        ...
        "docmeta_data": metadata_list,
          # Include the bounding box coordinates
    }
        ...
        if uploaded_file:
            if uploaded_file.type == "application/pdf":
                ...
                if uploaded_file and uploaded_file.type != "application/pdf":
                    ...
                    scale_factor = 1.3  # Adjust this factor as needed
                    new_width = int(img_cv.shape[1] * scale_factor)
                    new_height = int(img_cv.shape[0] * scale_factor)
                    img_resized = np.array(docImg.resize((new_width, new_height)))
                    ...
                    with col3_input_filed:
                        ...
                        col_radio, col_meta_input = st.columns([4, 6])
                        with col_radio:
                            active_field = st.radio(
                                "", [meta['metadata_type']['label'] for meta in metadata_types],
                                index=[meta['metadata_type']['label'] for meta in metadata_types].index(st.session_state.active_field),
                                key='active_field'
                            )
                        with col_meta_input:
                    ...
                    if canvas_result.json_data is not None:
                        ...
                        if objects:
                            ...
                            def get_extracted_text():
                                return pytesseract.image_to_string(roi, lang='eng').strip()
                            ...
                            for meta in metadata_types:
                                ...
                                if label == st.session_state.active_field:
                                    if st.session_state.inputs.get(input_key, "") != text:
